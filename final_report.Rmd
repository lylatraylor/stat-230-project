...
The model exhibits bias at the extremes, tending to overpredict low fuel loads and underpredict high fuel loads. The underprediction of high surface fuel loads is particularly concerning, as it could lead to underestimation of wildfire risk and insufficient management response.
...
As with the ridge regression, we assessed the adequacy of the linear model by visually inspecting the residual plot for curvilinear trends. Once again, we did not find concerning evidence of model misspecification (Fig. 3).

The LASSO model selected 11 variables (i.e., non-zero coefficients; Fig. 4), including site, the count of points in the 0-3 m high voxelized point clouds with tree stems and shrubs removed, five summary metrics on the entire voxelized point cloud, and four summary metrics on points classified as shrubs. 

In terms of predictive performance, the LASSO model performed slightly worse than the ridge model, with a higher RMSE (39.4 vs. 38.7 Mg/ha) and a lower $R^2$ (0.44 vs. 0.46) on the held-out test set. These results suggest that, while LASSO provides a more interpretable, sparse model, the ridge regression offers better overall predictive accuracy for this application.
...
```{r fig:myimage4, echo=FALSE, fig.cap="LASSO coefficient path plot. Dashed line indicates selected lambda value.", out.width="75%", fig.align="center"}
knitr::include_graphics("report_figs/lasso_coefs.png")
```

On our final ridge model, we also compute a leave-one-out conformal prediction interval at 90% confidence. This quantifies the predictive uncertainty for each observation in the training set. A traditional confidence interval characterizes the uncertainty around a population parameter such as the mean. A conformal prediction interval is instead constructed to contain the true response value for a new individual observation with a specified probability. It is also distribution-free and thus does not need to assume normality or homoskedasticity of the residuals.

The conformal intervals for our ridge model were quite wide, with a median width of 268.5 Mg/ha and a mean of 300.9 Mg/ha, with some intervals extending nearly 800 Mg/ha at the upper end of the fuel load range. This width reflects the variability in surface fuel load that the model cannot explain. This limits the practical utility of the intervals for precise prediction.

```{r fig:myimage5, echo=FALSE, fig.cap="Fitted line plot with conformal interval", out.width="75%", fig.align="center"}
knitr::include_graphics("report_figs/ridge_fitted_line_interval.png")
```
...
Lyla Traylor added the code for the ridge regression, wrote the code for the conformal prediction interval, and edited the report.

\newpage

## Appendix

```{r fig:myimage5, echo=FALSE, fig.cap="Terrestrial LiDAR point cloud, entire plot", out.width="75%", fig.align="center"}
knitr::include_graphics("report_figs/tls_scan_1.png")
```

```{r fig:myimage6, echo=FALSE, fig.cap="Terrestrial LiDAR point cloud, zoomed in", out.width="75%", fig.align="center"}
